...
describes what users get. Pick a format with `-o`, or let ronswanson choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
ronswanson <command> -o table   # aligned columns for reading
ronswanson <command> -o jsonl   # one JSON object per line, for piping
ronswanson <command> -o json    # a single JSON array
ronswanson <command> -o csv     # spreadsheet friendly
ronswanson <command> -o tsv     # tab-separated
ronswanson <command> -o url     # just the URL column
ronswanson <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
ronswanson <command> --fields id,title,url
```
...
```bash
ronswanson <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
ronswanson <command>            # a table, because this is a terminal
ronswanson <command> | wc -l    # JSONL, because this is a pipe
```
